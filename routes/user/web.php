...
Route::get('/verify-email', [UsersController::class, 'verifyemail'])->middleware('auth')->name('verification.notice');
;
...
Route::post('/email/verification-notification', function (Request $request) {
    try {
        ...
        return back()->with('error', 'Unable to send verification email. Please try again later.'); // Changed to 'error' or keep 'message' but with error text? Let's use 'message' for consistency if frontend expects it, or just use withInput. But usually flash messages are keyed. Let's assume 'message' is for success. I will add an error key.
    }
...
Route::middleware(['auth:sanctum', 'verified'])->prefix('dashboard')->group(function () {
    ...
    Route::middleware('complete.kyc')->group(function () {
        ...
        Route::get('deposit', [ViewsController::class, 'deposits'])->name('deposit');
        Route::get('signals', [ViewsController::class, 'signal'])->name('signal');
        Route::get('skip_account', [ViewsController::class, 'skip_account']);

        Route::get('trading-history', [ViewsController::class, 'tradinghistory'])->name('tradinghistory');
        Route::get('account-history', [ViewsController::class, 'accounthistory'])->name('accounthistory');
        ...
        Route::get('invest', [ViewsController::class, 'mplans'])->name('mplans');
        Route::get('portfolio', [ViewsController::class, 'myplans'])->defaults('sort', 'All')->name('myplans.default');
        Route::get('portfolio/{sort}', [ViewsController::class, 'myplans'])->name('myplans');
        ...
        //copytrading
        Route::get('explore-copytrading', [UserCopyTradingController::class, 'mcopytradings'])->name('mcopytradings');
        ...
        Route::prefix('social-copy')->name('copy.')->controller(CopyTradingController::class)->group(function () {
            Route::get('dashboard', 'dashboard')->name('dashboard');
            Route::get('portfolio', 'portfolio')->name('portfolio');
            Route::get('experts', 'experts')->name('experts');
            Route::get('experts/{slug}', 'show')->name('show');
            Route::post('watchlist/{traderId}', 'toggleWatchlist')->name('watchlist.toggle');
        ...
        Route::prefix('wallet')->name('wallet.')->controller(WalletController::class)->group(function () {
            ...
            Route::post('deposit', 'initiateDeposit')->name('deposit');
        });
        ...
        // Trading Routes
        Route::prefix('markets')->name('trade.')->controller(\App\Http\Controllers\User\TradeController::class)->group(function () {
            Route::get('/', 'index')->name('index');
        ...
        Route::match(['get', 'post'], 'newdeposit', [DepositController::class, 'newdeposit'])->name('newdeposit');
        Route::match(['get', 'post'], 'deposit/new', [DepositController::class, 'newdeposit'])->name('user.deposits.new');
        Route::get('payment', [DepositController::class, 'payment'])->name('payment');
        // Stripe save payment info
        Route::post('submit-stripe-payment', [DepositController::class, 'savestripepayment'])->name('submit-stripe-payment');
        ...
        Route::get('savedeposit', function () {
        ...
        // Withdrawals
        Route::match(['get', 'post'], 'enter-amount', [WithdrawalController::class, 'withdrawamount'])->name('withdrawamount');
        ...
        Route::post('exchange/process', [ExchangeController::class, 'exchange'])->name('user.exchange.process');
        ...
        Route::get('help', [ViewsController::class, 'support'])->name('user.support');

        Route::name('user.')->controller(MembershipController::class)->group(function () {
            Route::get('/courses', 'courses')->name('courses');
            Route::get('/course-details/{course}/{id}', 'courseDetails')->name('course.details');
            Route::post('/buy-course', 'buyCourse')->name('buycourse');
            Route::get('/my-courses', 'myCourses')->name('mycourses');
            Route::get('/my-courses/{id}', 'myCoursesDetails')->name('mycoursedetails');
            Route::get('/learning/{lesson}/{course?}', 'learning')->name('learning');
        });
