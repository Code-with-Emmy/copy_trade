...
class UserCopyTradingController extends Controller
{
    ...
    public function mcopytradings()
    {
        ...
        $userCopyTrades = [];
        if (Auth::check()) {
            $userCopyTrades = UserCopytrading::where('user', Auth::id())
                                              ->where('active', 'yes')
                                              ->pluck('cptrading')
                                              ->toArray();
        }
    ...
    public function copyTradingDashboard()
    {
        ...
        $copyTrades = UserCopytrading::where('user', $user->id)
                                      ->with('expert')
    ...
    public function joincopytrade(Request $request)
    {
        ...
        $existingCopy = UserCopytrading::where('user', $user->id)
        ...
        try {
            ...
            $copyTrade = UserCopytrading::create([
                'cptrading' => $expert->id,
                'user' => $user->id,
                'price' => $amount,
    ...
    public function stopCopyTrade($copyTradeId)
    {
        ...
        $copyTrade = UserCopytrading::where('id', $copyTradeId)
                                     ->where('user', $user->id)
                                     ->where('active', 'yes')
                                     ->first();
    ...
    public function automaticCopyTradingProfits()
    {
        ...
        $activeCopyTrades = UserCopytrading::where('active', 'yes')
                                            ->with(['user', 'expert'])
                                            ->get();
    ...
    public function cancelcopytrade(Request $request)
    {
        ...
        $user = Auth::user();
        $copyTrade = UserCopytrading::where('user', $user->id)
                                     ->where('active', 'yes')
                                     ->latest()
                                     ->first();
