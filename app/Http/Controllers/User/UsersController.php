...
class UsersController extends Controller
{
  ...
  public function sendcontact(ContactInquiryRequest $request)
  {
    ...
    $subject = "Inquiry from {$payload['name']} with email {$payload['email']}";

    try {
      Mail::to($settings->contact_email)->send(new NewNotification($message, $subject, 'Admin'));
    } catch (\Exception $e) {
      \Log::error('Failed to send contact inquiry email. Name: ' . $payload['name'] . ', Email: ' . $payload['email'] . ', Subject: ' . $subject . '. Error: ' . $e->getMessage());
    }
