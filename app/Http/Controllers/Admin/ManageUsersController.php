...
class ManageUsersController extends Controller
{
    ...
    public function viewuser($id)
    {
        ...
        $plans = $this->tableData('plans', fn () => Plans::where('type', 'Main')->get(), collect());
        $allPlans = $this->tableData('plans', fn () => Plans::orderByDesc('id')->get(), collect());
        $signals = $this->tableData('signals', fn () => Signal::where('type', 'Main')->get(), collect());
        $currencies = [];
        include app_path('Providers/currencies.php');

        $stats = [
            'deposits_total' => (float) $this->tableData('deposits', fn () => Deposit::where('user', $user->id)->sum('amount'), 0),
            'withdrawals_total' => (float) $this->tableData('withdrawals', fn () => Withdrawal::where('user', $user->id)->sum('amount'), 0),
            'plan_count' => (int) $this->tableData('user_plans', fn () => UserPlan::where('user', $user->id)->count(), 0),
            'copy_count' => (int) $this->tableData('user_copytradings', fn () => UserCopytrading::where('user', $user->id)->count(), 0),
            'bot_count' => (int) $this->tableData('user_bot_investments', fn () => UserBotInvestment::where('user_id', $user->id)->count(), 0),
            'activity_count' => (int) $this->tableData('activities', fn () => Activity::where('user', $user->id)->count(), 0),
        ];

        $recentDeposits = $this->tableData('deposits', fn () => Deposit::where('user', $user->id)->latest('id')->take(5)->get(), collect());
        $recentWithdrawals = $this->tableData('withdrawals', fn () => Withdrawal::where('user', $user->id)->latest('id')->take(5)->get(), collect());
        $recentActivities = $this->tableData('activities', fn () => Activity::where('user', $user->id)->latest('id')->take(6)->get(), collect());
    ...
    public function delsystemuser($id)
    {
        ...
        $userp = UserPlan::where('user', $id)->get();
        if (!empty($userp)) {
            foreach ($userp as $p) {
                //delete plans that their owner does not exist
                UserPlan::where('id', $p->id)->delete();
            }
        }
        ...
        $usersingal = UserSignal::where('user', $id)->get();
        if (!empty($usersingal)) {
            foreach ($usersingal as $p) {
                //delete plans that their owner does not exist
                UserSignal::where('id', $p->id)->delete();
            }
        }

        // delete user copy trading records
        $usercopytradings = UserCopytrading::where('user', $id)->get();
        if (!empty($usercopytradings)) {
            foreach ($usercopytradings as $copytrading) {
                UserCopytrading::where('id', $copytrading->id)->delete();
            }
        }
        ...
        $usertransactions = TpTransaction::where('user', $id)->get();
        if (!empty($usertransactions)) {
            foreach ($usertransactions as $transaction) {
                TpTransaction::where('id', $transaction->id)->delete();
            }
        }
    ...
    public function upgradesignalstatus(Request $request)
    {
        ...
        if (!empty($attributes)) {
            User::where('id', $request['user_id'])->update($attributes);
        }
    ...
    public function markprofit($id)
    {
        ...
        $trade = UserPlan::where('id', $id)->first();
