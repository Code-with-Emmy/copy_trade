...
class TradesController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $stats = [
            ...
            'active' => UserPlan::where('active', 'yes')->count(),
            'expired' => UserPlan::where('active', 'expired')->count(),
            'total_volume' => UserPlan::sum('amount')
        ];
    ...
    {
        try {
            $trade = UserPlan::with('user')->findOrFail($id);
    ...
    public function destroy($id)
    {
        try {
            DB::transaction(function () use ($id) {
                ...
                TpTransaction::where('plan', $id)->delete();
    ...
    public function getStats()
    {
        try {
            $stats = [
                'total_trades' => UserPlan::count(),
                'active_trades' => UserPlan::where('active', 'yes')->count(),
                'completed_trades' => UserPlan::where('active', 'expired')->count(),
                'total_volume' => UserPlan::sum('amount'),
                'total_profit' => UserPlan::sum('profit_earned'),
                'avg_trade_size' => UserPlan::avg('amount'),
                'recent_trades' => UserPlan::with('user')
    ...
    public function bulkAction(Request $request)
    {
        ...
        try {
            DB::transaction(function () use ($request) {
                ...
                switch ($action) {
                    case 'delete':
                        UserPlan::whereIn('id', $tradeIds)->delete();
                        TpTransaction::whereIn('plan', $tradeIds)->delete();
                        break;

                    case 'activate':
                        UserPlan::whereIn('id', $tradeIds)->update(['active' => 'yes']);
                        break;

                    case 'expire':
                        UserPlan::whereIn('id', $tradeIds)->update(['active' => 'expired']);
                        break;
                }
