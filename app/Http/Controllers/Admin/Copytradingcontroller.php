...
class Copytradingcontroller extends Controller
{
    ...
    public function trashcopytrading($id){
        ...
        $userscopytradings = UserCopytrading::where('cptrading', $id)->get();
        if (count( $userscopytradings) > 0) {
            foreach( $userscopytradings as $cpls){
                UserCopytrading::where('id', $cpls->id)->delete(); 
            }
    ...
    public function activecopytrading(){
        return view('admin.copytrading.activecopytrading',[
            ...
            'copytrades' => UserCopytrading::where('active', 'yes')->orderByDesc('id')->with(['dcopytrading', 'cuser'])->get(),
        ]);
    }
