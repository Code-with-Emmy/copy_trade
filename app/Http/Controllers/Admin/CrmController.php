...
class CrmController extends Controller
{
    public function addtask(Request $request)
    {
        ...
        $assigneeName = trim((string) data_get($mailduser, 'name', 'Admin'));

        try {
            Mail::to($mailduser->email)->send(new NewNotification($message, $subject, $assigneeName));
        } catch (\Exception $e) {
            \Log::error('Failed to send task assignment notification email. Admin: ' . $assigneeName . ' (' . $mailduser->email . '), Task: ' . $request->tasktitle . ', Task ID: ' . $task->id . '. Error: ' . $e->getMessage());
        }
