...
class AutoTaskController extends Controller
{
    ...
    */
    ...
    /**
     * Calculate and distribute automatic trading profits for UserPlan (trading)
     */
    public function automaticTradingProfit()
    {
        ...
        if ($settings->trade_mode == 'on') {
            // Get active trading plans (UserPlan)
            $tradingPlans = UserPlan::where('active', 'yes')->get();
            $now = now();
    ...
    public function automaticCopyTradingProfits()
    {
        ...
        if ($settings->trade_mode == 'on') {
            ...
            $activeCopyTrades = UserCopytrading::where('active', 'yes')->get();
            $now = Carbon::now();
            ...
            foreach ($activeCopyTrades as $copyTrade) {
                ...
                if ($now->greaterThanOrEqualTo($nextProfitTime)) {
                    ...
                    if ($profitResult['result'] == 'PROFIT') {
                        ...
                        UserCopytrading::where('id', $copyTrade->id)
                            ->update([
     ...
     * @param UserCopytrading $copyTrade
     * @return array
     */
