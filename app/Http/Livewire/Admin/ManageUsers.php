        ...
        DB::table('users')
            ...
            ->chunkById(100, function ($users) use ($plan) {
                foreach ($users as $user) {
                    $userplans = UserPlan::where('user', $user->id)->where('plan', $plan->id)->where('active', 'yes')->get();
                    if (count($userplans) > 0) {
                        ...
                        foreach ($userplans as $uplan) {
                            ...
                            $dplan = UserPlan::where('id', $uplan->id)->first();
    ...
    public function delsystemuser()
    {
        ...
        foreach ($users as $user) {
            ...
            if ($this->action == 'Delete') {
                ...
                $userp = UserPlan::where('user', $user->id)->get();
                if (!empty($userp)) {
                    foreach ($userp as $p) {
                 ...
                 $usersignals = UserSignal::where('user', $user->id)->get();
                ...
                $usercopytradings = UserCopytrading::where('user', $user->id)->get();
                ...
                $usertransactions = TpTransaction::where('user', $user->id)->get();
            ...
            if ($this->action == 'Clear') {
                ...
                if (!empty($usercopytradings)) {
                    foreach ($usercopytradings as $copytrading) {
                        UserCopytrading::where('id', $copytrading->id)->update([
                            'status' => 'inactive',
                            'profit_percentage' => 0,
                            'total_profit' => 0,
                        ]);
