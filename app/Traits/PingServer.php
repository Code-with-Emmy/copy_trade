...
trait PingServer
{
    ...
    public function apiResponseData(?Response $response, $default = [])
    {
        return data_get($response?->json(), 'data', $default);
    }

    public function apiResponseMessage(?Response $response, string $fallback = 'Request could not be completed.'): string
    {
        return (string) data_get($response?->json(), 'message', $fallback);
    }
