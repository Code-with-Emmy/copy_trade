...
trait Coinpayment
{
                ...
                if ($result['error'] == 'ok') {
                    ...
                    if ($status >= 100 || $status == 2) {
                        ...
                        try {
                            ...
                            \Log::error('Failed to send CoinPayments deposit notification email to admin. Deposit ID: ' . $dp->id . ', User: ' . $user->email . '. Error: ' . $e->getMessage());
                        }

                        try {
                            Mail::to($user->email)->send(new DepositStatus($dp, $user, 'Successful Deposit', false));
                        } catch (\Throwable $e) {
                            \Log::error('Failed to send CoinPayments deposit confirmation email to user. Deposit ID: ' . $dp->id . ', User: ' . $user->email . '. Error: ' . $e->getMessage());
                        }
    ...
    public function getMatchingBonus($sponsor, $amount)
    {
        ...
        foreach ($dwlns as $dwln) {
            ...
            if ($this->getDownlinePlans($dwln->id, $amount) == true) {
                ...
                $upln = UserPlan::where('id', $spnsr->user_plan)->first();
    ...
    {
        //get downline plans
        $plns = UserPlan::where('user', $dwln)->where('amount', $amount)->first();
