...
@section('content')
    ...
    <div class="page-content-stack animate-in fade-in slide-in-from-bottom-6 duration-1000" x-data="{ selectedFilter: 'All' }">
        <!-- Breadcrumb & Header -->
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
                ...
                <p class="text-slate-400 text-sm mt-1 font-medium">Monitoring {{ $numOfPlan }} investment
                    plans.</p>
            </div>
            ...
            <div class="flex items-center space-x-4">
                <div class="relative group">
                    <select x-model="selectedFilter"
                        @change="window.location.href = '{{ url('/dashboard/sort-plans') }}/' + selectedFilter"
                        ...
                        <option value="All" {{ request()->segment(3) == 'All' ? 'selected' : '' }}>All Assets</option>
                        <option value="yes" {{ request()->segment(3) == 'yes' ? 'selected' : '' }}>Active Only</option>
                        <option value="expired" {{ request()->segment(3) == 'expired' ? 'selected' : '' }}>Settled Plans
                        </option>
                    </select>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div class="dashboard-glass p-6 border-white/5 relative overflow-hidden group">
                ...
                <div class="text-3xl font-black text-white">{{ $numOfPlan }}</div>
                <div class="mt-2 flex items-center text-[10px] font-bold text-slate-500">
                    <span class="text-emerald-400 mr-1">+{{ $plans->where('active', 'yes')->count() }}</span> Active
                    plans
                </div>
            </div>
            ...
            <div class="dashboard-glass p-6 border-white/5 relative overflow-hidden group">
                ...
                <div class="text-3xl font-black text-white">
                    {{ Auth::user()->currency }}{{ number_format($plans->where('active', 'yes')->sum('amount'), 2) }}
                </div>
                Invested in <span class="text-white mx-1">{{ $plans->where('active', 'yes')->count() }}</span>
                plans
            </div>
        </div>
        ...
        <div class="dashboard-glass p-6 border-white/5 relative overflow-hidden group">
            ...
            <div class="text-3xl font-black gold-text">
                {{ Auth::user()->currency }}{{ number_format(Auth::user()->roi, 2) }}
            </div>
            Total profit earned
        </div>
    </div>
    ...
    <div class="dashboard-glass p-6 border-white/5 relative overflow-hidden group">
        ...
        <div class="text-3xl font-black text-white">{{ $plans->where('active', 'expired')->count() }}</div>
        Money unlocked from <span class="text-white mx-1">{{ $plans->where('active', 'expired')->count() }}</span> plans
    </div>
    </div>
    </div>
                        ...
                        <div class="flex items-center space-x-6 min-w-0 flex-1">
                            ...
                            <div class="min-w-0">
                                <div class="flex items-center space-x-3 mb-1">
                                    <h3 class="text-xl font-black text-white truncate">{{ $plan->uplan->name }}</h3>
                                    @if($plan->active == 'yes')
                                        <span
                                ...
                                <div class="flex items-center space-x-4 text-xs font-bold text-slate-500">
                                    <div class="flex items-center">
                                        ...
                                        <span
                                            class="text-slate-300">{{ Auth::user()->currency }}{{ number_format($plan->amount, 2) }}</span>
                                    </div>
                                    ...
                                    <div class="flex items-center">
                                        ...
                                        <span class="text-emerald-400">+{{ $plan->uplan->increment_amount }}%</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center space-x-8 lg:px-8 lg:border-l lg:border-r border-white/5">
                            ...
                            <div class="hidden sm:block">
                                ...
                                <div class="flex justify-between mt-2 text-[9px] font-black text-slate-600 uppercase">
                                    <span>{{ number_format($progress, 1) }}% Completed</span>
                                    <span>{{ $currentDate->diffInDays($endDate, false) > 0 ? $currentDate->diffInDays($endDate) . ' Days Left' : 'Completed' }}</span>
                                </div>
                            </div>
                            ...
                            <div class="text-center min-w-[100px]">
                                ...
                                <div class="text-lg font-black text-emerald-400 font-mono">
                                    +{{ Auth::user()->currency }}{{ number_format($plan->amount * ($plan->uplan->increment_amount / 100) * ($progress / 100), 2) }}
                                </div>
                ...
                <div
                    ...
                    <div class="flex items-center space-x-6">
                        <div class="flex items-center">
                            ...
                            Purchased: {{ $plan->created_at->format('M d, Y') }}
                        </div>
                        <div class="flex items-center">
                            <i data-lucide="calendar-check" class="w-3.5 h-3.5 mr-2 text-slate-600"></i>
                            Ends on: {{ \Carbon\Carbon::parse($plan->expire_date)->format('M d, Y') }}
                        </div>
                        <div class="flex items-center">
                            <i data-lucide="hash" class="w-3.5 h-3.5 mr-2 text-slate-600"></i>
                            ID: #BIT-{{ str_pad($plan->id, 5, '0', STR_PAD_LEFT) }}
                        </div>
                    </div>
    ...
    @if ($plans->hasPages())
        <div class="flex justify-center pt-10">
            {{ $plans->links('vendor.pagination.tailwind-custom') }}
        </div>
    @endif
    </div>
