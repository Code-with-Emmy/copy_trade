    ...
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
        @forelse ($plans as $plan)
            <div class="dashboard-glass border-white/5 hover:border-yellow-500/20 transition-all group relative overflow-hidden flex flex-col h-full"
                 :class="selectedPlanId === {{ $plan->id }} ? 'border-yellow-500/50 shadow-[0_0_30px_rgba(240,185,10,0.1)]' : ''">
                ...
                <div class="absolute top-6 right-6">
                    <div class="px-3 py-1.5 rounded-xl bg-emerald-500/10 border border-emerald-500/20 flex items-center space-x-2">
                        ...
                        <span class="text-[10px] font-black text-emerald-400 uppercase tracking-tighter">+{{ $plan->increment_amount }}% {{ $plan->increment_interval }}</span>
                    </div>
                </div>
                ...
                <div class="p-8 flex-1">
                    ...
                    <p class="text-slate-400 text-xs font-medium mb-8 leading-relaxed">
                        Advanced algorithmic execution model focused on {{ strtolower($plan->name) }} market dynamics and volatility harvesting.
                    </p>
                    ...
                    <div class="space-y-4 mb-10">
                        <div class="flex items-center justify-between text-xs">
                            ...
                            <span class="text-white font-black">{{ Auth::user()->currency }}{{ number_format($plan->min_price, 2) }}</span>
                        </div>
                        <div class="flex items-center justify-between text-xs">
                            <span class="text-slate-500 font-bold uppercase tracking-widest">Limit Cap</span>
                            <span class="text-white font-black">{{ Auth::user()->currency }}{{ number_format($plan->max_price, 2) }}</span>
                        </div>
                        <div class="flex items-center justify-between text-xs">
                            <span class="text-slate-500 font-bold uppercase tracking-widest">Duration Cycle</span>
                            <span class="text-white font-black">{{ $plan->expiration }}</span>
                        </div>
                        <div class="flex items-center justify-between text-xs">
                    ...
                    <form method="post" action="{{ route('joininvestmentplan') }}" class="space-y-4">
                        ...
                        <div x-data="{ amount: {{ $plan->min_price }} }">
                            <div class="relative group">
                                <label class="text-[9px] font-black text-slate-500 uppercase tracking-[0.2em] mb-2 block ml-1">Allocation Amount ({{ Auth::user()->currency }})</label>
                                <div class="bg-black/50 border border-white/10 rounded-xl p-1 focus-within:border-yellow-500/50 transition-all">
                                    <input type="number" name="iamount" x-model="amount" 
                                           min="{{ $plan->min_price }}" max="{{ $plan->max_price }}"
                                           @focus="selectedPlanId = {{ $plan->id }}"
                                ...
                                <div class="flex justify-between mt-2 px-1 text-[9px] font-bold text-slate-600 uppercase tracking-tighter">
                                    <span>Min: {{ number_format($plan->min_price) }}</span>
                                    <span>Max: {{ number_format($plan->max_price) }}</span>
                                </div>
                            </div>
                ...
                <div class="bg-black/40 px-8 py-4 border-t border-white/5 flex items-center justify-between text-[10px] font-black tracking-widest text-slate-500">
                    ...
                    <div class="flex items-center space-x-1">
                        @foreach(range(1,5) as $i)
                            <div class="h-1 w-3 rounded-full {{ $i <= 4 ? 'bg-yellow-500' : 'bg-white/10' }}"></div>
                        @endforeach
                    </div>
                </div>
            </div>
