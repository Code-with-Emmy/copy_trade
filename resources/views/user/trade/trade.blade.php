    ...
    <div class="page-content-stack animate-in fade-in slide-in-from-bottom-6 duration-1000" x-data="tradingMarkets()" x-cloak>
        ...
        <div class="dashboard-glass border-white/5 p-2 overflow-x-auto no-scrollbar">
            <div class="flex items-center space-x-2 min-w-max">
                ...
                @foreach (['crypto' => 'Cryptocurrency', 'stock' => 'Stocks', 'forex' => 'Forex', 'commodity' => 'Commodities', 'bond' => 'Bonds'] as $type => $label)
                    <button @click="selectedType = '{{ $type }}'" :class="selectedType === '{{ $type }}' ? 'bg-white/10 text-white border-white/10' :
                        ...
                        <i data-lucide="{{ $type === 'crypto' ? 'bitcoin' : ($type === 'stock' ? 'trending-up' : ($type === 'forex' ? 'globe' : ($type === 'commodity' ? 'zap' : 'landmark'))) }}"
                            class="w-4 h-4"></i>
                        <span>{{ $label }}</span>
                    </button>
                @endforeach
