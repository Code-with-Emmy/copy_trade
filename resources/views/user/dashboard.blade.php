                        ...
                        @foreach($assets as $asset)
                            <div
                                ...
                                <div class="text-right">
                                    <div class="text-xs font-mono font-black text-white leading-none">{{ $asset['price'] }}
                                    </div>
                                    <div
                                        class="text-[9px] font-black mt-1 {{ $asset['up'] ? 'text-emerald-400' : 'text-rose-400' }} uppercase tracking-widest">
                                        {{ $asset['change'] }}
                                    </div>
                                </div>
                            </div>
                    ...
                    <tbody class="divide-y divide-white/5">
                        @forelse($t_history as $history)
                            <tr class="group hover:bg-white/[0.02] transition-colors">
                                ...
                                <td class="px-8 py-6">
                                    <div class="flex items-center space-x-4">
                                        <div
                                            ...
                                            <i data-lucide="{{ $history->type == 'LOSE' ? 'trending-down' : 'trending-up' }}"
                                                class="w-4 h-4 {{ $history->type == 'LOSE' ? 'text-rose-500' : 'gold-text' }}"></i>
                                        </div>
                                ...
                                <td
                                    class="px-8 py-6 text-xs font-mono font-black {{ $history->type == 'LOSE' ? 'text-rose-400' : 'text-emerald-400' }}">
                                    {{ $history->type == 'LOSE' ? '-' : '+' }}{{ Auth::user()->currency }}{{ number_format($history->amount, 2) }}
                                </td>
                                <td class="px-8 py-6 text-[10px] font-bold text-slate-500 uppercase tracking-widest">
                                    {{ $history->created_at->format('M d, Y') }}<br>
                                    <span class="opacity-50 font-bold">{{ $history->created_at->format('H:i:s') }}</span>
                                </td>
