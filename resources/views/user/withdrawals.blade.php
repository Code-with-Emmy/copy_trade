    ...
    @if(Auth::user()->withdrawal_code === 'on')
        ...
        <div class="max-w-3xl mx-auto">
            <div class="dashboard-glass border-rose-500/20 p-6 sm:p-10 relative overflow-hidden text-center">
                ...
                <form action="{{ route('userwithdrawal') }}" method="post" class="space-y-6 max-w-sm mx-auto">
                    @csrf
                    ...
                    <a href="{{ route('support') }}" class="block text-[10px] font-black text-slate-500 uppercase tracking-widest hover:text-yellow-500 transition-colors pt-4">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 xl:gap-10">
                    ...
                    <form method="POST" action="{{ route('withdrawamount') }}" class="space-y-8">
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                @forelse ($wmethods as $index => $method)
                                    <label class="relative cursor-pointer group">
                                        <input type="radio" name="method" value="{{ $method->name }}" class="peer sr-only" x-model="selectedMethod" required>
                                        <div class="p-5 sm:p-6 rounded-2xl bg-white/5 border border-white/5 peer-checked:border-yellow-500/50 peer-checked:bg-yellow-500/10 hover:bg-white/10 transition-all flex items-center space-x-4">
                                            <div class="h-12 w-12 rounded-xl bg-black border border-white/10 flex items-center justify-center group-hover:scale-110 transition-transform">
                                                <i data-lucide="{{ str_contains(strtolower($method->name), 'bitcoin') ? 'bitcoin' : (str_contains(strtolower($method->name), 'bank') ? 'building-bank' : 'credit-card') }}" class="w-6 h-6 gold-text"></i>
                                            </div>
                                            <div>
                                                <div class="text-xs font-black text-white uppercase tracking-wider">{{ $method->name }}</div>
                                ...
                                @forelse ($withdrawals as $withdrawal)
                                    <tr class="text-xs font-bold group hover:bg-white/[0.02] transition-colors">
                                        <td class="px-5 sm:px-8 py-5 text-slate-400 flex items-center">
                                            <div class="w-2 h-2 rounded-full mr-3 {{ $withdrawal->status == 'Processed' ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]' : 'bg-yellow-500' }}"></div>
                                            #TX-{{ str_pad($withdrawal->id, 6, '0', STR_PAD_LEFT) }}
                                        </td>
                                        <td class="px-4 sm:px-6 py-5 text-center text-white">{{ Auth::user()->currency }}{{ number_format($withdrawal->amount, 2) }}</td>
                                        <td class="px-4 sm:px-6 py-5 text-center">
                                            <span class="px-2 py-1 rounded-md bg-white/5 text-slate-400 text-[9px] uppercase tracking-tighter border border-white/5">{{ $withdrawal->payment_mode }}</span>
                                        </td>
            ...
            <div class="lg:col-span-4 space-y-5 sm:space-y-6">
                ...
                <div class="dashboard-glass border-white/5 p-6 sm:p-8 overflow-hidden relative group">
                    ...
                    <a href="{{ route('support') }}" class="block w-full text-center py-3 rounded-xl bg-white/5 border border-white/10 text-[10px] font-black text-slate-300 uppercase tracking-widest hover:bg-yellow-500 hover:text-black transition-all">
                        Contact Support
                    </a>
                </div>
            </div>
