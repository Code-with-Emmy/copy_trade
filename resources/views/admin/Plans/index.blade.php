    ...
    <div class="page-content-stack animate-in fade-in slide-in-from-bottom-6 duration-1000">
        ...
        <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-4">
            <div class="dashboard-glass p-6 sm:p-7">
                ...
                <h2 class="mt-4 text-3xl font-black tracking-tighter text-white">{{ number_format($plans->count()) }}</h2>
            ...
            <div class="dashboard-glass p-6 sm:p-7">
                ...
                <h2 class="mt-4 text-3xl font-black tracking-tighter gold-text">{{ number_format($activePlansCount) }}</h2>
            ...
            <div class="dashboard-glass p-6 sm:p-7">
                ...
                <h2 class="mt-4 text-3xl font-black tracking-tighter text-white">{{ number_format($featuredPlansCount) }}</h2>
            ...
            <div class="dashboard-glass p-6 sm:p-7">
                ...
                <h2 class="mt-4 text-3xl font-black tracking-tighter text-white">{{ number_format($averageRoi, 2) }}%</h2>
        ...
        <div class="dashboard-glass overflow-hidden">
            <div class="flex flex-col gap-4 border-b border-white/5 bg-white/[0.02] p-6 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div class="rounded-2xl border border-white/5 bg-black/30 px-4 py-3 text-right">
                    ...
                    <p class="mt-1 text-sm font-bold text-white">{{ $plans->count() > 0 ? number_format(($featuredPlansCount / $plans->count()) * 100, 0) : 0 }}% of catalog highlighted</p>
                </div>
            </div>
                            ...
                            <tr class="transition-colors hover:bg-white/[0.03]">
                                <td class="px-6 py-4 align-top">
                                    <div class="flex items-center gap-3">
                                        @if ($plan->icon)
                                            <img src="{{ asset('storage/' . $plan->icon) }}" alt="{{ $plan->name }}" class="h-11 w-11 rounded-2xl border border-white/10 object-cover">
                                        @else
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs text-slate-400">{{ $plan->roi_interval }}</p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 align-top text-slate-300">{{ optional($plan->category)->name ?? 'No Category' }}</td>
                                <td class="px-6 py-4 align-top text-slate-300">
                                    {{ $settings->currency }}{{ number_format((float) $plan->min_amount, 2) }}
                                    <span class="text-slate-600">to</span>
                                    {{ $settings->currency }}{{ number_format((float) $plan->max_amount, 2) }}
                                </td>
                                <td class="px-6 py-4 align-top font-black text-white">{{ number_format((float) $plan->roi_percentage, 2) }}%</td>
                                <td class="px-6 py-4 align-top text-slate-300">{{ $plan->duration }} {{ $plan->duration_unit }}</td>
                                <td class="px-6 py-4 align-top">
                                    <form action="{{ route('admin.plans.toggle', $plan) }}" method="POST">
                                        @csrf
                                        <button type="submit"
                                            class="inline-flex rounded-full px-3 py-1 text-[10px] font-black uppercase tracking-widest {{ $plan->is_active ? 'bg-emerald-500/10 text-emerald-400' : 'bg-rose-500/10 text-rose-300' }}">
                                            {{ $plan->is_active ? 'Active' : 'Inactive' }}
                                        </button>
                                    </form>
                                </td>
                                <td class="px-6 py-4 align-top">
                                    <span class="inline-flex rounded-full px-3 py-1 text-[10px] font-black uppercase tracking-widest {{ $plan->is_featured ? 'bg-yellow-500/10 text-yellow-400' : 'bg-white/5 text-slate-400' }}">
