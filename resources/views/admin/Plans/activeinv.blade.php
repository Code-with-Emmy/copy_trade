                        ...
                        <table id="ShipTable" class="table table-hover ">
                            ...
                            <tbody>
                                @foreach ($plans as $plan)
                                    <tr>
                                        <td>{{ $plan->duser->name ?? 'User Deleted' }}</td>
                                        <td>{{ $plan->dplan->name ?? 'Plan Deleted' }}</td>
                                        <td>{{ $plan->duser->currency ?? '' }}{{ number_format($plan->amount) }}</td>
                                        <td>{{ $plan->inv_duration }}</td>
                                        <td>
                                            {{ $plan->duser->currency ?? '' }}{{ $plan->profit_earned ? $plan->profit_earned : '0' }}
                                        </td>
                                            ...
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                                    ...
                                                    <a class="dropdown-item"
                                                        href="{{ route('user.plans', $plan->duser->id ?? "1") }}">More
                                                        actions</a>
                                                </div>
                                            </div>
                                        </td>
