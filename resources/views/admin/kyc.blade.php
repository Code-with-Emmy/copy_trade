    ...
    <div class="page-content-stack animate-in fade-in slide-in-from-bottom-6 duration-1000">
        ...
        <div class="grid grid-cols-1 gap-5 sm:grid-cols-3">
            <div class="dashboard-glass p-6 sm:p-7">
                ...
                <h2 class="mt-4 text-3xl font-black tracking-tighter text-white">{{ number_format($kycs->count()) }}</h2>
            ...
            <div class="dashboard-glass p-6 sm:p-7">
                ...
                <h2 class="mt-4 text-3xl font-black tracking-tighter text-emerald-400">{{ number_format($verifiedCount) }}</h2>
            ...
            <div class="dashboard-glass p-6 sm:p-7">
                ...
                <h2 class="mt-4 text-3xl font-black tracking-tighter gold-text">{{ number_format($pendingCount) }}</h2>
                    ...
                    <tbody class="divide-y divide-white/5">
                        @forelse ($kycs as $list)
                            <tr class="transition-colors hover:bg-white/[0.03]">
                                <td class="px-6 py-4 font-bold text-white">{{ data_get($list, 'user.name', 'Deleted User') }}</td>
                                <td class="px-6 py-4">
                                    <span class="inline-flex rounded-full px-3 py-1 text-[10px] font-black uppercase tracking-widest {{ $list->status === 'Verified' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-yellow-500/10 text-yellow-400' }}">
                                        {{ $list->status }}
                                    </span>
                                </td>
